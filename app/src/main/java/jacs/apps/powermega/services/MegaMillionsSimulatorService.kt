...
import jacs.apps.powermega.MainActivity
...
class MegaMillionsSimulatorService : Service() {
    private val mBinder: IBinder = MegaMillionsSimulatorServiceBinder()
    private var listener: MegamillionsSimulatorListener? = null
    var thread: Thread? = null
    var handler = Handler()
    var serviceRunnable: Runnable = object : Runnable {
        override fun run() {
            if (update) {
                data = SharedPrefHelper.getSimData(this@MegaMillionsSimulatorService, Constants.MEGA_SIM)
                counter = SharedPrefHelper.getLongMegaCounter(this@MegaMillionsSimulatorService)
                update = false
            }
            val random = Random()
            val nums = ArrayList<Int>()
            while (nums.size < 6) {
                val num = random.nextInt(70) + 1
                if (!nums.contains(num)) {
                    nums.add(num)
                }
            }
            val power = random.nextInt(25) + 1
    ...
    fun setMegaMillionsSimulatorListener(thelistener: MegamillionsSimulatorListener?) {
    ...
    fun setUpForeground() {
        ...
        val serviceIntent = Intent(this, MegaMillionsSimulatorService::class.java)
        serviceIntent.putExtra("shutdown", "shutdown")
        val pendingServiceShutdown = PendingIntent.getService(this, 40, serviceIntent, 0)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val name: CharSequence = "Powerball"
            val description = "Powerball Notification"
            val importance = NotificationManager.IMPORTANCE_DEFAULT
            val channel = NotificationChannel("101010101010", name, importance)
        ...
        val notification = NotificationCompat.Builder(this, "101010101010")
                ...
                .build()
        startForeground(Constants.MEGA_SIM_FOREGROUND, notification)
        running = true
    ...
    override fun onDestroy() {
        ...
        SharedPrefHelper.setSimData(this, data, Constants.MEGA_SIM)
        SharedPrefHelper.setLongMegaCounter(this, counter)
        Log.d("powerservice", "service destroyed")
    }

    inner class MegaMillionsSimulatorServiceBinder : Binder() {
        // Return this instance of LocalService so clients can call public methods
        val service: MegaMillionsSimulatorService
            get() =// Return this instance of LocalService so clients can call public methods
                this@MegaMillionsSimulatorService
    }
